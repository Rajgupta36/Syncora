  ...
  const columns: GridColDef[] = [
    { field: 'title', headerName: 'Title', width: 200 },
    {
      ...
      renderCell: (prev) => {
        return (
          <span
            className={`h-5 px-1 py-0.5 rounded text-sm font-normal text-center ${statusColorMap[prev.value.toLowerCase() as keyof typeof statusColorMap]}`}
          >
    ...
    },
    {
      ...
      renderCell: (params) => (
        <span
          className={`h-5 px-1 py-0.5 rounded text-sm font-normal text-center ${priorityColorMap[params.value.toLowerCase() as keyof typeof priorityColorMap]}`}
        >
    ...
    },
    {
      ...
      renderCell: (prev) => (
        <span className="text-sm font-normal">
          {format(new Date(prev.value), 'MM/dd/yyyy')}
        </span>
      ),
    },
    {
      ...
      renderCell: (prev) => (
        <span className="text-sm font-normal">
          {prev.value ? prev.value : 'Not Assigned'}
        </span>
      ),
    },
  ];
